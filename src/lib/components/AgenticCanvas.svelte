<script lang="ts">
  ...
  interface TagData  { label: string; color: string; }
  ...
  const LAYERS = [
    { num: '1', color: '#818cf8', title: 'Client Layer',
    ...
    { num: '2', color: '#fbbf24', title: 'Standards & Frameworks Layer',
    ...
    { num: '3', color: '#fcd34d', title: 'AI Gateway Layer',
    ...
    { num: '4', color: '#2dd4bf', title: 'Observability Layer',
  ...
  const SECTION_HEADING = 'font-size:10px; font-weight:700; color:#64748b; letter-spacing:2px; text-transform:uppercase; margin:0 0 14px';
</script>

<!-- ── Reusable snippets ─────────────────────────────────────── -->

{#snippet tagBadge(label: string, color: string)}
  <span style="display:inline-block; background:{color}22; border:1px solid {color}55; border-radius:4px; font-size:9.5px; padding:1px 6px; font-family:monospace; font-weight:700; color:{color}">
    {label}
  </span>
{/snippet}

{#snippet numBadge(n: string, color: string)}
  <div style="width:30px; height:30px; border-radius:50%; border:2px solid {color}; background:{color}18; display:flex; align-items:center; justify-content:center; font-size:13px; font-weight:800; color:{color}; flex-shrink:0">
    {n}
  </div>
{/snippet}

{#snippet psBadge()}
  <span style="display:inline-flex; align-items:center; gap:4px; background:linear-gradient(90deg,#7c3aed44,#4f46e544); border:1px solid #818cf8aa; color:#c4b5fd; font-size:8.5px; font-weight:800; text-transform:uppercase; letter-spacing:1px; padding:2px 8px; border-radius:9999px">
    ★ Personal Standard
  </span>
{/snippet}
...
{#snippet depBadge(type: DepType)}
  ...
  <span style="display:inline-block; background:{s.bg}; border:1px solid {s.border}; color:{s.color}; font-size:8.5px; font-weight:700; padding:2px 8px; border-radius:9999px">
    {s.label}
  </span>
{/snippet}
...
{#snippet divider(label: string)}
  <div style="display:flex; align-items:center; gap:12px; margin:20px 0 16px">
    ...
    <span style="font-size:9.5px; color:#475569; font-weight:700; letter-spacing:2px; text-transform:uppercase">{label}</span>
    <div style="flex:1; height:1px; background:rgba(255,255,255,0.07)"></div>
  </div>
{/snippet}
  ...
  <div style={bandStyle(p.fill, p.isPS)}>
    ...
    {#if p.bullets && p.bullets.length > 0}
      <ul style="margin:0; padding:0 0 0 12px; display:flex; flex-direction:column; gap:2px">
        {#each p.bullets as b}<li style="font-size:10px; color:#94a3b8; line-height:1.5">{b}</li>{/each}
      </ul>
    {/if}
...
{#snippet cell(p: CellProps, children?: Snippet)}
  <div style={cellStyle(p.fill, p.isPS)}>
    <div style="display:flex; align-items:flex-start; gap:10px">
      ...
      <div>
        <div style="font-size:13px; font-weight:800; color:{p.badge}; line-height:1.2">{p.title}</div>
        {#if p.subtitle}<div style="font-size:11px; color:#94a3b8; margin-top:2px">{p.subtitle}</div>{/if}
      </div>
    </div>
    {#if p.isPS}{@render psBadge()}{/if}
    {#if p.bullets && p.bullets.length > 0}
      <ul style="margin:0; padding:0 0 0 14px; display:flex; flex-direction:column; gap:3px">
        {#each p.bullets as b}<li style="font-size:11px; color:#94a3b8; line-height:1.5">{b}</li>{/each}
      </ul>
    {/if}
    {#if p.tags && p.tags.length > 0}
  ...
  <div class="grid2" style="gap:8px; margin-top:4px">
    <div>
      ...
      <div style="display:flex; flex-direction:column; gap:6px">
        {@render bandCell({ fill: PALETTE.col5a.fill, badge: PALETTE.col5a.badge, title: 'Filesystem MCP', subtitle: 'Read/write project files',    deps: ['Local'] })}
        {@render bandCell({ fill: PALETTE.col5a.fill, badge: PALETTE.col5a.badge, title: 'Git MCP',        subtitle: 'Branch, diff, commit, log',    deps: ['Local'] })}
        {@render bandCell({ fill: PALETTE.col5a.fill, badge: PALETTE.col5a.badge, title: 'Docker MCP',     subtitle: 'Container management',         deps: ['Local'] })}
        {@render bandCell({ fill: PALETTE.col5a.fill, badge: PALETTE.col5a.badge, title: 'Postgres MCP',   subtitle: 'Query + schema inspection',    deps: ['Local'] })}
      </div>
    </div>
    <div>
      {@render divider('Remote')}
      <div style="display:flex; flex-direction:column; gap:6px">
        {@render bandCell({ fill: PALETTE.col5a.fill, badge: PALETTE.col5a.badge, title: 'Prefect MCP',    subtitle: 'Workflow orchestration',       deps: ['LocalRemote'] })}
        {@render bandCell({ fill: PALETTE.col5a.fill, badge: PALETTE.col5a.badge, title: 'Databricks MCP', subtitle: 'Data platform integration',    deps: ['LocalRemote'] })}
        {@render bandCell({ fill: PALETTE.col5a.fill, badge: PALETTE.col5a.badge, title: 'Kubernetes MCP', subtitle: 'EKS / GKE cluster ops',        deps: ['LocalRemote'] })}
        {@render bandCell({ fill: PALETTE.col5a.fill, badge: PALETTE.col5a.badge, title: 'ArgoCD MCP',     subtitle: 'GitOps deployment',            deps: ['LocalRemote'] })}
      </div>
    </div>
  </div>
{/snippet}
...
{#snippet agentsContent()}
  <div class="grid2" style="gap:8px; margin-top:4px">
    <div>
      {@render divider('Local')}
      <div style="display:flex; flex-direction:column; gap:6px">
        {@render bandCell({ fill: PALETTE.col5b.fill, badge: PALETTE.col5b.badge, title: 'Claude Code CLI',  subtitle: 'Agentic coder — primary dev agent', deps: ['Rules', 'Skills'], isPS: true })}
        {@render bandCell({ fill: PALETTE.col5b.fill, badge: PALETTE.col5b.badge, title: 'HolmesGPT',        subtitle: 'Incident investigator agent',        deps: ['Local'] })}
      </div>
    </div>
    <div>
      {@render divider('Remote / CI')}
      <div style="display:flex; flex-direction:column; gap:6px">
        {@render bandCell({ fill: PALETTE.col5b.fill, badge: PALETTE.col5b.badge, title: 'PR-Agent (Qodo)',  subtitle: 'Automated code review',             deps: ['LocalRemote'] })}
        {@render bandCell({ fill: PALETTE.col5b.fill, badge: PALETTE.col5b.badge, title: 'GitLab Duo',       subtitle: 'CI-integrated AI coder',            deps: ['LocalRemote'] })}
        {@render bandCell({ fill: PALETTE.col5b.fill, badge: PALETTE.col5b.badge, title: 'GitHub Copilot',   subtitle: 'IDE + PR AI assistance',            deps: ['LocalRemote'] })}
        {@render bandCell({ fill: PALETTE.col5b.fill, badge: PALETTE.col5b.badge, title: 'Metis (Arm)',      subtitle: 'Security agent for CI',             deps: ['LocalRemote'] })}
      </div>
    </div>
  </div>
{/snippet}
...
<div style="background:{PALETTE.bg}; min-height:100vh; font-family:{typography.fontSans}">
  <div style="padding:40px 18px 22px">
    ...
    <div style="margin-bottom:28px; text-align:center">
      <div style="font-size:9px; font-weight:700; color:#475569; letter-spacing:2px; text-transform:uppercase; margin-bottom:6px">
    ...
    <div style="background:rgba(255,255,255,0.03); border:1px solid rgba(255,255,255,0.07); border-radius:12px; padding:16px; margin-bottom:20px">
      ...
      <div class="grid4" style="gap:12px">
        {#each LAYERS as l}
          <div style="background:{l.color}10; border:1px solid {l.color}33; border-radius:8px; padding:12px 14px; display:flex; flex-direction:column; gap:8px">
    ...
    <div style="margin-bottom:20px">
      ...
      <div style="background:{PALETTE.gw.fill}10; border:1.5px solid {PALETTE.gw.fill}55; border-radius:12px; padding:14px 16px">
        <div class="grid5" style="gap:10px">
          {@render bandCell({ fill: PALETTE.gw.fill, badge: PALETTE.gw.badge,
    ...
    <div style="margin-bottom:20px">
      ...
      <div style="background:{PALETTE.obs.fill}10; border:1.5px solid {PALETTE.obs.fill}55; border-radius:12px; padding:14px 16px">
        <div class="grid4" style="gap:10px">
          {@render bandCell({ fill: PALETTE.obs.fill, badge: PALETTE.obs.badge,
    ...
    <!-- ── Tools Reference ────────────────────────────────────── -->
    <div style="background:rgba(255,255,255,0.03); border:1px solid rgba(255,255,255,0.04); border-radius:12px; padding:16px; margin-bottom:20px; overflow:visible">
...
<style>
  ...
  :global(.grid4)  { display: grid; grid-template-columns: 1fr 1fr 1fr 1fr; }
  :global(.grid3)  { display: grid; grid-template-columns: 1fr 1fr 1fr; }
  :global(.grid5)  { display: grid; grid-template-columns: 1fr 1fr 1fr 1fr 1fr; }
  :global(.grid2)  { display: grid; grid-template-columns: 1fr 1fr; }
  ...
  @media (max-width: 1024px) {
    :global(.grid5)  { grid-template-columns: 1fr 1fr 1fr; }
    .grid-tools      { grid-template-columns: repeat(4, 1fr); }
  }

  @media (max-width: 768px) {
    :global(.grid4)  { grid-template-columns: 1fr 1fr; }
    :global(.grid3)  { grid-template-columns: 1fr 1fr; }
    :global(.grid5)  { grid-template-columns: 1fr 1fr; }
    :global(.grid2)  { grid-template-columns: 1fr; }
    .grid-tools      { grid-template-columns: repeat(3, 1fr); }
  }

  @media (max-width: 480px) {
    :global(.grid4)  { grid-template-columns: 1fr; }
    :global(.grid3)  { grid-template-columns: 1fr; }
    :global(.grid5)  { grid-template-columns: 1fr; }
    :global(.grid2)  { grid-template-columns: 1fr; }

# indest_labs.dev — Full Rebuild Specification
...
- **Domain**: indest_labs.dev
- **Owner**: Indest Labs — Agentic Institutional Engineer · AI Systems
- **Purpose**: Personal website — hero, writing, certifications, education, tools canvas
- **Repository**: GitHub Pages static site (`awindest/agentic-canvas.github.io`)
...
ignore this line - `public/CNAME` — content: `indest_labs.dev`
...
- Left: brand wordmark "indest_labs.dev" in `#818cf8`, `fontWeight: 800`, `fontSize: 15`; links to `/`
...
**`<Head>`:** title `"Indest Labs — Platform Engineering & AI Systems"`, viewport meta, favicon.
...
- H1: `"Indest Labs"` — `fontSize: 40`, `fontWeight: 900`, `color: #f1f5f9`, `margin: 0 0 16px`, `lineHeight: 1.15`
...
Content: `indest_labs.dev · RFCA Solutions · {new Date().getFullYear()}`
...
**`<Head>`:** `<meta name="viewport" content="width=device-width, initial-scale=1" />`, title `"GenAI Technology Stack Canvas — indest_labs.dev"`.
...
1. **Title block** — "DEVELOPED BY RAPHAEL MORAES</br>MODIFIED BY INDEST LABS" label, H1 "GenAI Technology Stack Setup Canvas", subtitle "Opinionated, layered AI-native engineering stack for Vibe Coders · v1.1", legend row (Personal Standard + 4 dep badge types)
...
8. **Footer** — centered, `fontSize: 10`, `color: #334155`, text: `indest_labs.dev · GenAI Engineering Standard v1.0 · 2026`
...
After build, verify:
- `out/CNAME` exists (contains `indest_labs.dev`)
- `out/.nojekyll` exists
